...
CBaseEntity* CFollowbot::FindTarget(CBaseEntity* pLocal)
{
	for (const auto& pPlayer : g_EntityCache.GetGroup(EGroupType::PLAYERS_TEAMMATES))
	{
		...
		if (Vars::Misc::Paste::Followbotpookies.Value && !g_EntityCache.IsFriend(pPlayer->GetIndex())) { continue; }
...
void CFollowbot::Run(CUserCmd* pCmd)
{
	...
	if (!Vars::Misc::Paste::Followbot.Value || !pLocal || !pLocal->IsAlive())
	{
		Reset();
		return;
	}
	...
	if (!PathNodes.empty())
	{
		...
		{
			if (localPos.DistTo(CurrentTarget->GetAbsOrigin()) >= Vars::Misc::Paste::Followbotdistance.Value)
			{
...
{
	if (!Vars::Misc::Paste::Followbot.Value || !CurrentTarget) { return; }
